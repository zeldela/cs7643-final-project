...
def get_volumetric_augmentations():
    return A.Compose([
        A.HorizontalFlip(p=0.5),
        A.VerticalFlip(p=0.5),
    ])
...
class CustomDataset(Dataset):
    def __init__(self, images ,cfg,xyxys=None, labels=None, transform=None, volumetric_augmentations=None, valid=None):
        self.images = images
        self.cfg = cfg
        self.labels = labels
        ...
        self.volumetric_augmentations = volumetric_augmentations
        self.valid = valid

    def __len__(self):
        return len(self.images)

    def vol_augment(self, image):
      if self.volumetric_augmentations and not self.valid:
            augmented = self.volumetric_augmentations(image=image)
...
for fid in fragments:
    ...
    volumetric_augmentations = get_volumetric_augmentations()
